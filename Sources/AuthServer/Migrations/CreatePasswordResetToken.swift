...
/// Fluent migration that creates the `password_reset_tokens` table.
///
...
public struct CreatePasswordResetToken: AsyncMigration {
    ...
    /// Creates the `password_reset_tokens` table with a unique constraint on `token`
    ...
    /// Drops the `password_reset_tokens` table, reversing the migration.
    ///
    /// - Parameter database: The database connection to run the schema change on.
